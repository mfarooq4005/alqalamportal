...
import { OrbitControls, Sphere, MeshDistortMaterial, Stars, Float, Box, Torus, Icosahedron } from '@react-three/drei';
import { motion } from 'framer-motion';
import { Link } from 'react-router-dom';
import { ArrowRight, Award, Users, BookOpen, TrendingUp, Cpu, Atom, Microscope, Palette, Calculator, Rocket, GraduationCap } from 'lucide-react';
...
const RobotHead = () => {
  ...
  useFrame((state) => {
    ...
    if (eyeLeftRef.current && eyeRightRef.current) {
      const blink = Math.sin(state.clock.elapsedTime * 3) > 0.95;
  ...
  return (
    <group ref={headRef}>
      {/* Head */}
      <Box args={[1.2, 1.4, 1]} position={[0, 0, 0]}>
        <MeshDistortMaterial color="#0ea5e9" distort={0.1} speed={1} roughness={0.3} metalness={0.8} />
      </Box>
      {/* Eyes */}
      <Box args={[0.25, 0.15, 0.1]} position={[-0.3, 0.2, 0.5]} ref={eyeLeftRef}>
        <MeshDistortMaterial color="#f97316" emissive="#f97316" emissiveIntensity={2} />
      </Box>
      <Box args={[0.25, 0.15, 0.1]} position={[0.3, 0.2, 0.5]} ref={eyeRightRef}>
        <MeshDistortMaterial color="#f97316" emissive="#f97316" emissiveIntensity={2} />
      </Box>
      {/* Antenna */}
      <Torus args={[0.15, 0.05, 16, 32]} position={[0, 0.9, 0]} rotation={[Math.PI / 2, 0, 0]}>
        <MeshDistortMaterial color="#a855f7" metalness={0.9} roughness={0.2} />
      </Torus>
      <Box args={[0.05, 0.3, 0.05]} position={[0, 1.05, 0]}>
        <MeshDistortMaterial color="#f97316" emissive="#f97316" emissiveIntensity={1} />
      </Box>
    </group>
  );
};

// STREAM Subject Icons in 3D
const StreamIcon = ({ position, color, icon: Icon, label }) => (
  <Float speed={3} rotationIntensity={2} floatIntensity={1.5}>
    <group position={position}>
      <Icosahedron args={[0.4, 0]} scale={1.2}>
        <MeshDistortMaterial color={color} transparent opacity={0.7} distort={0.2} speed={2} metalness={0.6} roughness={0.3} />
      </Icosahedron>
      <Html distanceFactor={10}>
        <div className="bg-white/90 backdrop-blur-sm px-2 py-1 rounded-lg text-xs font-bold shadow-lg whitespace-nowrap">
          {label}
        </div>
      </Html>
    </group>
...
const AnimatedSphere = () => {
  ...
  return (
    <Sphere args={[1, 100, 200]} scale={2.2} ref={meshRef}>
      <MeshDistortMaterial color="#0ea5e9" attach="material" distort={0.4} speed={2} roughness={0.2} metalness={0.8} />
    </Sphere>
  );
};
